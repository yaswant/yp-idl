...
;           ASCII files in loop.
; Syntax:
;           build_ascii_template [,FILE=String] [,/FHEADER]
;               [,FIELD_NAMES=Strarr] [,SAVE_TFILE=String]
; Arguments:
;           None
; Keywords:
...
;           (e.g., . / \ * ! # @, etc) in the Field names array.
...
pro build_ascii_template, FILE=file, FHEADER=fheader, $
  ...
  for i=0,n-1 do begin
    ...
    ; Print tag names and elements to stdout
    print,' ',tags[i],' : ',s,(i lt n-1)?',$':'}'

    ; Store structure values in a string array in order to save as a template
    ; function
  ...
  ; Save ASCII tamplate as an IDL function; once build, the function can be
  ; called from any IDL procedure
  ; ---------------------------------------------------------------------------
