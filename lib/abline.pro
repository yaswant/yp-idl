...
;    XLIMIT (IN:Array) Crop limits of the straight line in X direction;
;                      default is plot limit
;    YLIMIT (IN:Array) Crop limits of the straight line in Y direction;
;                      default is plot limit
;    /XLOG For logarithmic X-axis
;    /YLOG For logarithmic Y-axis
;    /REVERSE Plot line in reverse order (from Right->Left or Top->Bottom)
...
; :
; - - - - - - - - - - - - - - - - - - - - - - - - - -
; :COPYRIGHT: Yaswant Pradhan (c) Crown Copyright Met Office
; :HISTORY:
...
pro abline, H=h, V=v, A=a, B=b, DEGREE=degree, XLIMIT=xlim, YLIMIT=ylim, $
            XLOG=xlog, YLOG=ylog, REVERSE=reverse, _EXTRA=_extra
    ...
    if N_ELEMENTS(h) gt 0 then begin
        ...
        for i=0, N_ELEMENTS(h)-1 do begin
            yArray = xArray*bb + aa     ; get the Y array from line equation
    ...
    if N_ELEMENTS(v) gt 0 then begin
        ...
        for i=0, N_ELEMENTS(v)-1 do begin
            xArray = yArray*bb + aa     ; get the X array from line equation
