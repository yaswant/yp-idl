...
;-
  ...
  ; Longitudes:
    ...
    if (n_east gt 0) then out[east] = format_values(array[east], dp)+$
                                      deg+'E'    
    if (n_west gt 0) then out[west] = format_values(abs(array[west]), dp)+$
                                      deg+'W'
  ...
  ; Latitudes:
    ...
    if (n_north gt 0) then out[north] = format_values(array[north], dp)+$
                                        deg+'N'  
    if (n_south gt 0) then out[south] = format_values(abs(array[south]), dp)+$
                                        deg+'S'  
