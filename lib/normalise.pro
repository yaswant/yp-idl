FUNCTION normalise, data, BOXCOX=BoxCox, $
                    MANLY=Manly, JOHNDRAPER=JohnDraper, $
                    YEOJOHNSON=YeoJohnson, TEST=test, DIAG=diag, $
                    GET_LAMBDA=opt_lambda

;+
; NAME:
;     NORMALISE (use of 's' in normalise is intentional);
;       Note: This is different to David Fanning's NORMALIZE function 
;       which is used to calculate the scaling vector required
;       to position a graphics primitive of specified range
;       at a specific position in an arbitray coordinate system.
; 
; PURPOSE:
;     Normalise a skewed data series using four different power transformation 
;     methods:
...
; 
; SYNTAX:
;     Result = NORMALISE( Data | /TEST [,/BOXCOX | ,/MANLY | ,/JOHNDRAPER | 
;                       ,/YEOJOHNSON] [,/DIAG] [,GET_LAMBDA=variable] )
; 
; ARGUMENTS:
;     data: 1D array of any type except string
; 
; KEYWORDS:
;     BOXCOX:     Applies calssical BoxCox (1964) power transformation (def)
...
;-
  ...
  if( n_params() lt 1 and ~keyword_set(test) ) then $
    message,' Syntax: result = NORMALISE( Data | /TEST [,/BOXCOX | ,/MANLY | '+$
         ',/JOHNDRAPER | ,/YEOJOHNSON] [,/DIAG] [,GET_LAMBDA=variable] )'
...
; Generate normally distributed data, 0 mean 1 variance 
; (use normal quantile function)
  for p=0,lines-1 do qnorm(p) = gauss_cvf(points(p))
...
; -------------------------------------------------------------------------
  if keyword_set(diag) then begin
    device, decomposed=0
    window, 1, xs=750,ys=450, title='Normalise', xpos=1,ypos=1
    show_plot     = 1b    
    !p.color      = '000000'x
...
;     window, 1, xs=750,ys=450, title='Normalise', xpos=0,ypos=0
    ...
    plot, x,hist_data,psym=10, title='Histogram of Original data', $
...
; -------------------------------------------------------------------------
  if ~(keyword_set(BoxCox) or keyword_set(Manly) or $
       keyword_set(JohnDraper) or keyword_set(YeoJohnson)) then begin

    method='Box-Cox'
; If -ve values are present in the data
    if(min(data) lt 0.) then begin
      print,' WARNING! -ve values found in the data, '+$
            'an offset of (min + offset) will be added to the data'
...
; Find lambda corresponding to the max-correlation iteratively
    ...
    for i=0,n_lambda-1 do begin
      ...
      if( lambda(i) ne 0. ) then $
        trans_data = (data^lambda(i) - 1.)/lambda(i) $
      else $
        trans_data = alog(data)
...
; Calculate transformed data for optimal lambda
    ...
    if ( opt_lambda ne 0. ) then $
      opt_trans_data = (data^opt_lambda - 1.)/opt_lambda $
    else $
      opt_trans_data = alog(data)
...
; Find lambda corresponding to the max-correlation iteratively
    ...
    for i=0,n_lambda-1 do begin
      lambda(i) = (i-n_half)/100.0D
      if( lambda(i) ne 0. ) then $ 
        trans_data = ( exp( data*lambda(i) ) - 1. ) / lambda(i) $
      else $
        trans_data = data
...
; Calculate transformed data for optimal lambda
    ...
    if ( opt_lambda ne 0. ) then $
      opt_trans_data = ( exp( data*opt_lambda ) - 1. ) / opt_lambda $
    else $
      opt_trans_data = data
...
; -------------------------------------------------------------------------
  if (keyword_set(JohnDraper) or strcmp(default_method,'John-Draper',/fold)) then begin
    method='John-Draper'
...
;Find lambda corresponding to the max-correlation iteratively
  ...
  ;sign=make_array(lines,value=-1)
    ...
    for i=0,n_lambda-1 do begin
      lambda(i) = (i-n_half)/100.0D
      if( lambda(i) ne 0. ) then $
        trans_data = sign * ( ((abs(data)+1)^lambda(i)) - 1. ) / lambda(i) $
      else $
        trans_data = sign * alog(abs(data)+1)
...
; Calculate transformed data for optimal lambda
    ...
    if ( opt_lambda ne 0. ) then $
      opt_trans_data = sign * ( ((abs(data) + 1)^opt_lambda) - 1.0 ) / opt_lambda $
...
;Find lambda corresponding to the max-correlation iteratively
    ...
    for i=0,n_lambda-1 do begin
      ...
      for k=0,lines-1 do begin
        if( (lambda(i) ne 0.) and (data(k) ge 0.) ) then $
          trans_data(k) = ( ((data(k)+1.)^lambda(i)) - 1.0 ) / lambda(i) $
        else if( (lambda(i) eq 0.) and (data(k) ge 0.) ) then $
          trans_data(k) = alog( data(k)+1. ) $
        else if( (lambda(i) ne 2.) and (data(k) lt 0.) ) then $
          trans_data(k) = ( ((1.- data(k))^(2.-lambda(i))) - 1.0 ) / (lambda(i) - 2.) $
        else if( (lambda(i) eq 2.) and (data(k) lt 0.) ) then $
          trans_data(k) = -( alog(1. - data(k)) ) $
        else $
          trans_data(k) = data(k)
...
; Calculate transformed data for optimal lambda
    ...
    for k=0,lines-1 do begin
      if( (opt_lambda ne 0.) and (data(k) ge 0.) ) then $
        opt_trans_data(k) = ( ((data(k)+1.)^opt_lambda) - 1.0 ) / opt_lambda $
      else if( (opt_lambda eq 0.) and (data(k) ge 0.) ) then $
        opt_trans_data(k) = alog( data(k)+1. ) $
      else if( (opt_lambda ne 2.) and (data(k) lt 0.) ) then $
        opt_trans_data(k) = ( ((1.- data(k))^(2. - opt_lambda)) - 1.0 ) / (opt_lambda - 2.) $
      else if( (opt_lambda eq 2.) and (data(k) lt 0.) ) then $
        opt_trans_data(k) = -( alog(1. - data(k)) ) $
      else $
        opt_trans_data(k) = data(k)
    endfor
...
; -------------------------------------------------------------------------
  if(show_plot) then begin
    plot, lambda, corr, psym=1, symsize=0.6, yst=1, $
          title=method+'Normality plot', xtitle='lambda',ytitle='r!u2'
    oplot,[opt_lambda,opt_lambda],[0,1]

    hist_trans_data=histogram(opt_trans_data, nbins=12, location=trans_x)
    plot, trans_x,hist_trans_data,psym=10, $
          title='Histogram of Transformed data', xtitle='t(X, lambda)',ytitle='frequency'

    plot, reverse(qnorm),data[sort(data)],psym=4, symsize=.5, yst=1, $
          title='Normal Q-Q Plot', xtitle='Standard Normal Quantiles', $
          ytitle='Ordered Original data'
    
    fit = linfit(reverse(qnorm),data[sort(data)],yfit=fit_data)
    oplot,reverse(qnorm),fit_data

    plot, reverse(qnorm),opt_trans_data[sort(opt_trans_data)], psym=4,symsize=.5, $
          yst=1, title='Normal Q-Q Plot', xtitle='Standard Normal Quantiles',$
          ytitle='Ordered Transformed data'
