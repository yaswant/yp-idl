...
;               [,XOUT=variable] [,YOUT=variable] [,/DOUBLE] [,/FAST])
; 
; INPUTS:
...
; Parse Input --
  ...
  message,'[grid_xyz]: X, Y, Z array must have equal number of elements.'
...
; Rescale X and Y vector and Construct X, Y, Z arrays (2D)
  ...
  ; Loop through all input elements
    print,'[grid_xyz]: Resampling '+strtrim(n_z,2)+' scattered points to ['+$
           strtrim(nX,2)+','+strtrim(nY,2)+'] grid...'
