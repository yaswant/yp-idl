FUNCTION get_h5, Filename, Object, $
...
;
; :PURPOSE:
;     GET_H5 function extracts a single dataset from a valid HDF5 file. The 
...
;
; :SYNTAX:
;     Result = GET_H5(Filename, Object [,START=Array] [,COUNT=Array] 
...
;
; IDL> data = get_h5('MSG_200802111015_lite.h5', '/MSG/IR_039/Raw', $
...
;  26-Jul-2012 Stable and renamed to a generic form from get_msg_h5. YP.
;
;-

  !QUIET=1
  syntax ='Result = GET_H5( Filename, Object '+$
...
; Valid Hdf File? Status Code: -1  
  if ~H5F_IS_HDF5(Filename) then begin
    message,'Not a Valid H5 File.',/CONTINUE
    status = -1
    return, status
  endif
